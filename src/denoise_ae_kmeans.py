...
class DenoisingAutoencodedKMeans(object):
    def __init__(self, data, labels, num_clusters, embed_dim, epochs, noisy_data,  batch_size=100, nonlinear=tf.nn.tanh):
        self.raw_images = noisy_data
        self.epochs = epochs
        ...
        noisy_images = tf.train.slice_input_producer([noisy_data], num_epochs=self.epochs)
        noisy_images = tf.squeeze(tf.train.batch([noisy_images], batch_size=self.batch_size))
        noisy_images = tf.cast(train_images, tf.float32)
        ...
        #train graph
        #autoencoder
        embed_x = self.nonlinear(tf.matmul(noisy_images, input_weights) + input_bias)
        ...
        #self.opt = tf.train.AdagradOptimizer(learning_rate=1e-3).minimize(self.loss)
            ...
            while not self.coord.should_stop():
                ...
                if step % 500 == 0: 
                    ...
                    print('%s\t%0.4f\t%0.4f\t%0.4f\t%0.4f\t%0.4f\t%0.4f' 
                            % (step, ars(self.labels, assignments), mis(self.labels, assignments), 
                              v_score(self.labels, assignments), self.purity_score(self.labels, assignments), norm, sum / step))
...
def main():
    ...
    noise = np.random.normal(0,varience,images.shape)
    noisy_data = np.clip(images + noise, 0, 255)
    
    kmeans = DenoisingAutoencodedKMeans(data=images, labels=labels, num_clusters=100, embed_dim=128, epochs=200, noisy_data=noisy_data, batch_size=100, nonlinear=tf.nn.tanh)
    print '\nVARIANCE: %s' % str(varience)
    kmeans.train()
